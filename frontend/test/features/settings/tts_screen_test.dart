...
class FakeHiveManager with ChangeNotifier implements HiveManager {
  FakeHiveManager({String initialGender = '남성', String initialSpeed = '보통'}) {
    _fakeSettings =
        FakeAppSettings(ttsGender: initialGender, ttsSpeed: initialSpeed);
  }
  ...
  @override
  Future<void> updateAccessibility({bool? highContrast, bool? reduceMotion, bool? emphasizeCaptions}) async {}
  @override
  Future<void> updateTagColorTheme(String theme) async {}
  @override
  List<HiveSubject> getSubjects({bool favoritesOnly = false, List<String> filterTagIds = const []}) => [];
  @override
  HiveSubject? getSubject(String id) => null;
  @override
  Future<void> toggleSubjectFavorite(String id) async {}
  @override
  Future<void> updateSubject(String id, {String? title, bool? favorite, List<String>? tagIds, List<String>? lectureIds}) async {}
  @override
  ...
  @override
  Future<void> updateLectureMetadata(String id, {String? weekLabel, String? title}) async {}
  @override
...
void main() {
  ...
  Future<void> pumpTtsScreen(WidgetTester tester,
  ...
  group('tts_screen.dart: Widget Test', () {
    group('1. UI Initial State Verification (Mock Data -> UI)', () {
      // (로딩 테스트는 동일)
      testWidgets('로딩 중에는 CircularProgressIndicator가 표시되어야 함',
          (tester) async {
      ...
      testWidgets('설정이 "여성", "빠르게"일 때 UI에 반영되어야 함', (tester) async {
        fakeHiveManager = FakeHiveManager(initialGender: '여성', initialSpeed: '빠르게');
        await pumpTtsScreen(tester, locale: const Locale('ko'));
      ...
      testWidgets('AppBar 타이틀이 "TTS"이고 닫기 버튼이 있어야 함', (tester) async {
        await pumpTtsScreen(tester);
        expect(find.descendant(of: find.byType(AppBar), matching: find.text('TTS')), findsOneWidget);
    ...
    group('2. User Interaction Verification (UI -> Logic & State)', () {
      ...
      testWidgets('"여성" 버튼 탭 시 updateTts(gender: "여성") 호출 및 UI 변경', (tester) async {
      ...
      testWidgets('영어 "Male" 버튼 탭 시 updateTts(gender: "남성") 호출', (tester) async {
      ...
      testWidgets('영어 "Female" 버튼 탭 시 updateTts(gender: "여성") 호출', (tester) async {
        await pumpTtsScreen(tester, locale: const Locale('en'));
      ...
      testWidgets('"빠르게" 버튼 탭 시 updateTts(speed: "빠르게") 호출 및 UI 변경', (tester) async {
      ...
      testWidgets('"느리게" 버튼 탭 시 updateTts(speed: "느리게") 호출 및 UI 변경', (tester) async {
        await pumpTtsScreen(tester, locale: const Locale('ko'));
