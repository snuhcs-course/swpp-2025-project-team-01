...
class FakeBackgroundService extends Fake implements BackgroundService {
  ...
  bool enableBackgroundExecutionCalled = false;
  bool disableBackgroundExecutionCalled = false;
  ...
  Future<void> enableBackgroundExecution() async {
    enableBackgroundExecutionCalled = true;
  }

  @override
  Future<void> disableBackgroundExecution() async {
    disableBackgroundExecutionCalled = true;
  }
}
...
void main() {
  ...
  group('2. Form Input & Interaction', () {
    ...
    testWidgets('Verify tapping AppBar back button calls Navigator.pop', (
      WidgetTester tester,
    ) async {
      // 테스트용 고유 키(Key) 생성
  ...
  group('3. Audio File List Management', () {
    ...
    testWidgets('Tapping Remove Audio (-) with file shows dialog and removes', (
      ...
      final secondAudioButton = find.widgetWithText(OutlinedButton, '추가').at(2);
  ...
  group('5. Create Button - Success & Failure Logic', () {
    ...
    testWidgets('Tapping Create on Success saves to Hive and shows toast', (
      ...
      final fakeResult = CreationResult(
        ...
        duration: 12345,
        originalAudioPath: '/result/original.m4a',
      );
    ...
    testWidgets(
      'Tapping Create on Cancellation shows no toast and does not save',
      (WidgetTester tester) async {
        // [Given] 폼이 유효하고, 가짜 서비스가 취소(null)를 반환하도록 설정
        final l10n = await pumpScreen(tester);
        await fillValidForm(tester);

        fakeCreator.setCancel(); // null 반환
        fakeLoader.setCancelled(true); // 로더가 취소됨 상태
    ...
    testWidgets('Verify loader service is called during creation', (
      WidgetTester tester,
    ) async {
      // [Given] 폼이 유효하고, 성공 결과 설정
      await pumpScreen(tester);
      await fillValidForm(tester);
      ...
      // 로더 호출 횟수 초기화
      fakeLoader.startLoadingCalled = false;
      fakeLoader.hideLoadingCalled = false;
      ...
      // [Then] 로더 서비스 메서드들이 호출됨
      expect(fakeLoader.startLoadingCalled, isTrue);
      // Note: hideLoading은 LectureLoadingService에서 자동으로 처리되므로
      // 여기서는 startLoading만 검증
    });

    testWidgets('Verify background service is initialized during creation', (
      WidgetTester tester,
    ) async {
      // [Given] 폼이 유효하고, 성공 결과 설정
      await pumpScreen(tester);
      ...
      // 백그라운드 호출 플래그 초기화
      fakeBackground.initializeCalled = false;
      fakeBackground.enableBackgroundExecutionCalled = false;
      ...
      // [Then] 백그라운드 서비스가 초기화되고 활성화됨
      expect(fakeBackground.initializeCalled, isTrue);
      expect(fakeBackground.enableBackgroundExecutionCalled, isTrue);
    });

    testWidgets(
      'Verify cancel callback is registered and handles cancellation',
      (WidgetTester tester) async {
        // [Given] 폼이 유효하고, 성공 결과 설정
        final l10n = await pumpScreen(tester);
        await fillValidForm(tester);

        // 강의 생성을 무한 대기하도록 설정 (취소를 테스트하기 위해)
        fakeCreator.setCancel();

        // 백그라운드 초기화
        fakeBackground.initializeCalled = false;
        fakeBackground.enableBackgroundExecutionCalled = false;
        fakeBackground.disableBackgroundExecutionCalled = false;

        // [When] 생성하기 버튼 탭 (백그라운드에서 실행)
        final createButton = find.text(l10n.isKorean ? '생성하기' : 'Create');
        await tester.ensureVisible(createButton);
        await tester.tap(createButton);

        // pump만 하고 settle은 하지 않음 (비동기 작업이 진행 중)
        await tester.pump();

        // [When] 사용자가 취소 버튼 클릭 (콜백 트리거)
        fakeLoader.triggerCancel();
        await tester.pumpAndSettle();
        ...
        // 2. 백그라운드 실행이 비활성화됨
        expect(fakeBackground.disableBackgroundExecutionCalled, isTrue);
  ...
  // ------------------------------------------------------------------
  group('6. State Management & Utilities', () {
  ...
  // ------------------------------------------------------------------
  // 테스트 케이스 그룹 9: Multi-Audio Page Range Edge Cases
  // ------------------------------------------------------------------
  group('9. Multi-Audio Page Range Edge Cases', () {
  ...
  // ------------------------------------------------------------------
  // 테스트 케이스 그룹 10: Edge Cases for File Uploads
  // ------------------------------------------------------------------
  group('10. Edge Cases for File Uploads', () {
